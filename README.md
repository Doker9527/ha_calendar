...
4. 打开 **设置 → 仪表盘 → 资源**，添加：
   ...
   ```text
   /chinese-calendar/chinese-calendar-card.js?v=0.1.6
   ```
...
在仪表盘中添加一张“手动”卡片，切换到 YAML 编辑，粘贴下面配置。仪表盘显示日期、农历与实时翻页时钟的一体化入口；点击整张卡打开完整万年历。翻页时钟只在仪表盘入口显示，弹窗内不显示。

需要先安装 Browser Mod 2.6 或更新版本，并在 **设置 → 仪表盘 → 资源** 中加入 `/chinese-calendar/chinese-calendar-card.js?v=0.1.6`，资源类型选择“JavaScript 模块”。若已经添加旧资源，编辑原条目的版本号，不要重复新增。手机上还需完全关闭 Home Assistant App 或浏览器标签页后重新打开，以清除旧前端脚本。
