...
本文件用于在另一台电脑上继续开发。仓库：<https://github.com/Doker9527/ha_calendar>，默认分支 `main`。当前代码版本为 `0.1.6`。不要把 Home Assistant 登录信息、令牌或本地配置文件提交到此公开仓库。
...
- `dashboard/chinese-calendar-entry.yaml`：可直接粘贴到 Home Assistant 仪表盘的一体化日期、农历、翻页时钟入口及 Browser Mod 弹窗配置。
- `README.md`：安装、资源地址和使用说明。

Home Assistant 仪表盘资源地址为 `/chinese-calendar/chinese-calendar-card.js?v=0.1.6`，类型为 JavaScript 模块。弹窗使用 `custom:chinese-calendar-card`，无需将 HACS 的 `update` 实体填入卡片。
