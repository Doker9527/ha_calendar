...
class ChineseCalendarCard extends HTMLElement {
  ...
  _render() {
    ...
    const body = compact
      ? `<div class="compact-calendar">${this._renderDateSummary(day)}${this._renderCalendar()}</div>`
      : `<div class="almanac-layout"><div class="upper-layout">${details ? this._renderLeft(day) : ""}<main class="center-column">${this._renderDateSummary(day)}${this._renderCalendar()}</main>${details ? this._renderRight(day) : ""}</div>${this._renderFortuneStrip(day)}${details ? this._renderDirections(day) : ""}${details ? this._renderMobileDetails(day) : ""}</div>`;
    this.shadowRoot.innerHTML = `<style>${this._styles()}</style><ha-card class="calendar-card ${compact ? "compact-card" : ""}">${header}${this._error ? `<div class="message error">${this._escape(this._error)}</div>` : ""}${this._loading && !this._data ? '<div class="message">正在计算本月农历…</div>' : ""}${day ? body : ""}</ha-card>`;
  ...
  _styles() {
    return `
      ...
      *{box-sizing:border-box}button{font:inherit}.calendar-card{display:block;width:100%;overflow:hidden;padding:clamp(12px,1.5vw,20px);border:1px solid var(--line);border-radius:var(--ha-card-border-radius,24px);background:var(--surface);box-shadow:var(--ha-card-box-shadow,0 8px 28px rgba(30,39,56,.07))}.card-title{display:flex;align-items:center;justify-content:space-between;margin:0 0 14px 3px}.card-title h2{margin:0;font-size:26px;font-weight:720;letter-spacing:.02em}.close-button{width:34px;height:34px;border:0;color:var(--muted);background:transparent;font-size:34px;font-weight:300;line-height:1;cursor:pointer}.close-button:hover{color:var(--text)}
      .almanac-layout{display:flex;min-width:0;flex-direction:column;gap:10px}.compact-card{max-width:1100px;margin-inline:auto}.compact-calendar{display:flex;min-width:0;flex-direction:column;gap:10px}.upper-layout{display:grid;min-height:720px;grid-template-columns:minmax(240px,.82fr) minmax(500px,1.75fr) minmax(230px,.78fr);grid-template-rows:minmax(0,1fr);align-items:stretch;gap:10px}.center-column{display:flex;min-width:0;min-height:0;flex-direction:column;gap:10px}.side{display:flex;min-width:0;min-height:0;flex-direction:column;gap:10px}.date-summary,.calendar-shell,.fortune-strip,.directions,.info-panel{border:1px solid var(--line);border-radius:13px;background:color-mix(in srgb,var(--panel) 48%,var(--surface));box-shadow:0 2px 8px rgba(25,35,50,.025)}
      ...
      @media(max-width:1200px) and (min-width:901px){.calendar-card{padding:14px}.upper-layout{grid-template-columns:minmax(210px,.9fr) minmax(0,1.45fr)}.right-side{display:none}.days{grid-template-rows:repeat(6,minmax(48px,1fr))}.solar{font-size:15px}}
      @media(max-width:900px){.calendar-card{padding:12px;border-radius:var(--ha-card-border-radius,22px)}.card-title{margin-bottom:10px}.card-title h2{font-size:21px}.upper-layout{display:flex;height:auto;flex-direction:column}.center-column{order:1}.left-side,.right-side{display:none}.side{grid-template-columns:1fr 1fr;grid-template-rows:auto}.date-summary{padding:8px 12px}.date-row{min-height:25px;gap:10px;font-size:13px}.date-row b{width:34px}.date-row strong{font-size:15px}.calendar-shell{padding:5px}.day{min-height:49px}.solar{font-size:14px}.lunar{font-size:10px}.fortune-days{overflow-x:auto;grid-template-columns:repeat(13,44px);padding-bottom:3px}.directions>div{grid-template-columns:repeat(5,1fr)}}
      @media(max-width:540px){.card-title{display:flex;margin:0 8px 10px}.card-title h2{font-size:22px}.close-button{width:30px;height:30px;font-size:30px}.calendar-card{padding:8px;border:0;border-radius:0;background:color-mix(in srgb,var(--panel) 70%,var(--surface));box-shadow:none}.almanac-layout,.upper-layout,.center-column,.mobile-details{gap:7px}.date-row{flex-wrap:wrap}.date-row em{margin-left:auto}.month-nav{grid-template-columns:32px 1fr auto 1fr 32px;font-size:13px}.weekdays div{padding:6px 1px;font-size:11px}.days{grid-template-rows:repeat(6,minmax(45px,1fr))}.day{min-height:45px;padding:4px 1px}.solar{font-size:13px}.lunar{font-size:9px}.date-summary,.calendar-shell,.fortune-strip,.directions,.mobile-tile{border-radius:14px;background:var(--surface)}.mobile-details{display:flex;flex-direction:column}.mobile-tile{gap:6px;padding:9px 10px}.mobile-tile b{font-size:14px}.mobile-tile span{font-size:11px}.desktop-directions{display:none}.mobile-directions{display:block}.fortune-strip h3{font-size:16px}.fortune-strip h3 small{display:block;margin:3px 0 0;font-size:11px}.fortune-row{gap:2px}.fortune-arrow{width:24px;font-size:26px}.fortune-days{min-width:0;overflow:hidden;grid-template-columns:repeat(13,minmax(0,1fr));gap:4px;padding:0}.fortune-day{display:none}.fortune-day:nth-child(n+5):nth-child(-n+9){display:flex}.directions{padding:9px}.directions span{font-size:10px}}
