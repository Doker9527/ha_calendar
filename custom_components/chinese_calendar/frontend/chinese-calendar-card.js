...
class ChineseCalendarCard extends HTMLElement {
  ...
  _renderMobileDetails(day) {
    const block = (title, body, tone = "") => `<div class="mobile-block ${tone}"><b>${this._escape(title)}</b><div class="mobile-block-body">${body || "—"}</div></div>`;
    const tile = (...blocks) => `<section class="mobile-tile">${blocks.join("")}</section>`;
    const line = (label, value, note = "") => `<div class="mobile-fact"><b>${this._escape(label)}</b><span>${this._escape(value)}${note ? `<small>${this._escape(note)}</small>` : ""}</span></div>`;
    const zodiac = line("生肖年", `${day.year_zodiac}（${day.year_ganzhi}）`, day.year_nayin) + line("生肖月", `${day.month_zodiac}（${day.month_ganzhi}）`, day.month_nayin) + line("生肖日", `${day.day_zodiac}（${day.day_ganzhi}）`, day.day_nayin);
    const conflict = line("相冲", `冲${day.chong}`) + line("岁煞", day.sha) + line("天神", `${day.tian_shen}（${day.tian_shen_type}）`);
    const terms = this._data.solar_terms?.length ? this._data.solar_terms.map((term) => line(term.name, `${term.date} 周${term.weekday}`)).join("") : "本月无节气数据";
    const nextTerm = this._data.solar_terms?.find((term) => term.date > day.date) || this._data.solar_terms?.[0];
    const nextTermBody = nextTerm ? line(nextTerm.name, `${nextTerm.date} 周${nextTerm.weekday}`) : "本月无节气数据";
    return `<div class="mobile-details"><div class="mobile-grid">${tile(block("宜", this._join(day.yi), "good"))}${tile(block("忌", this._join(day.ji), "bad"))}${tile(block("生肖 · 纳音", zodiac, "accent"))}${tile(block("值日 · 冲煞", conflict, "bad"))}${tile(block("本月节气", terms, "accent"))}${tile(block("下次节气", nextTermBody, "accent"))}${tile(block("吉神宜趋", this._join(day.ji_shen, "—"), "good"))}${tile(block("凶煞宜忌", this._join(day.xiong_sha, "—"), "bad"))}${tile(block("彭祖百忌", this._escape(day.pengzu), "bad"))}${tile(block("胎神", `${this._escape(day.month_tai_shen)}　${this._escape(day.tai_shen)}`, "bad"))}${tile(block("日禄", this._escape(day.day_lu), "bad"), block("物候", `${this._escape(day.hou)}　${this._escape(day.wu_hou)}`, "bad"))}${tile(block("九星", this._escape(day.nine_star), "bad"), block("星宿", this._escape(day.xiu), "bad"))}</div>${this._renderDirections(day, true)}</div>`;
  }
  ...
  _styles() {
    return `
      ...
      *{box-sizing:border-box}button{font:inherit}.calendar-card{display:block;width:100%;overflow:hidden;padding:clamp(12px,1.5vw,20px);border:1px solid var(--line);border-radius:var(--ha-card-border-radius,24px);background:var(--surface);box-shadow:var(--ha-card-box-shadow,0 8px 28px rgba(30,39,56,.07))}.card-title{display:flex;align-items:center;justify-content:space-between;margin:0 0 14px 3px}.card-title h2{margin:0;font-size:26px;font-weight:720;letter-spacing:.02em}.close-button{width:34px;height:34px;border:0;color:var(--muted);background:transparent;font-size:34px;font-weight:300;line-height:1;cursor:pointer}.close-button:hover{color:var(--text)}
      ...
      .mobile-tile{display:flex;flex-direction:column;gap:9px;align-items:stretch}.mobile-block{display:flex;min-width:0;flex-direction:column;gap:5px}.mobile-block+.mobile-block{padding-top:8px;border-top:1px solid var(--line)}.mobile-block>b{font-size:14px;line-height:1.35;white-space:normal}.mobile-block.good>b{color:var(--good)}.mobile-block.bad>b{color:var(--bad)}.mobile-block.accent>b{color:var(--accent)}.mobile-block-body{min-width:0;font-size:12px;line-height:1.55;overflow-wrap:anywhere}.mobile-fact{display:grid;grid-template-columns:43px minmax(0,1fr);gap:4px;margin-bottom:5px}.mobile-fact:last-child{margin-bottom:0}.mobile-fact>b{font-size:11px;line-height:1.45;color:var(--muted);font-weight:600;white-space:nowrap}.mobile-fact>span{display:block;min-width:0;font-size:11px;line-height:1.45;white-space:normal;overflow-wrap:anywhere}.mobile-fact small{display:block;color:var(--muted);font-size:10px}
