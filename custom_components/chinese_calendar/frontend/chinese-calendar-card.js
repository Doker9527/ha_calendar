...
class ChineseCalendarCard extends HTMLElement {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `<style>${this._styles()}</style><ha-card class="calendar-card">${header}${this._error ? `<div class="message error">${this._escape(this._error)}</div>` : ""}${this._loading && !this._data ? '<div class="message">正在计算本月农历…</div>' : ""}${day ? `<div class="almanac-layout"><div class="upper-layout">${details ? this._renderLeft(day) : ""}<main class="center-column">${this._renderDateSummary(day)}${this._renderCalendar()}</main>${details ? this._renderRight(day) : ""}</div>${this._renderFortuneStrip(day)}${details ? this._renderDirections(day) : ""}${details ? this._renderMobileDetails(day) : ""}</div>` : ""}</ha-card>`;
